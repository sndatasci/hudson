...
Series::NAREITDriver::NAREITDriver(void):
  _linenum(0)
{
}
...
bool Series::NAREITDriver::open(const std::string& filename)
{
...
void Series::NAREITDriver::close(void)
{
  if( _infile.is_open() )
	  _infile.close();
...
bool Series::NAREITDriver::next(DayPrice& dp) throw(Series::DriverException)
{
  if( _infile.eof() )
  	return false;
...
bool Series::NAREITDriver::eof(void)
{
  return _infile.eof();
}
